...
namespace Logify.Mobile.Views {
    public partial class RawReportPage : ContentPage {
        public RawReportPage() {
            InitializeComponent();
        }
        public RawReportPage(string apiKey, string reportId):this() {
            Task.Run(async () => {
                IReportDetailDataProvider provider = DataProviderFactory.CreateReportDetailDataProvider();
                string rawReport = await provider.LoadRawReport(apiKey, reportId);
                Device.BeginInvokeOnMainThread(() => {
                    this.BindingContext = new RawReportViewModel(rawReport);
