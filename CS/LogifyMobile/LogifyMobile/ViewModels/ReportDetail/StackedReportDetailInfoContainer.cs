...
namespace Logify.Mobile.ViewModels.ReportDetails {
    public class StackedReportDetailInfoContainer : ScrolledContentDetailInfoContainer {
        ...
        public StackedReportDetailInfoContainer(List<object> values) : base(values) {
        }

        protected override void ProcessValue(object item, bool isLastCard) {
            if (item is JObject) {
                ExceptionInfo exception = ((JObject)item).ToObject<ExceptionInfo>();
                if (exception != null) {
                    if (Values == null) {
                        Values = new List<ExceptionInfoModel>();
                    }
                    Values.Add(new ExceptionInfoModel(exception));
                }
            }
        }
    }
