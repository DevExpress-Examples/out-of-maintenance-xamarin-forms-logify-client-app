...
namespace Logify.Mobile.ViewModels.Statistics {
    public class CountByStatusesViewModel : NotificationObject {
        ...
        public void UpdateStatuses() => Task.Run(async () => {
            var statistics = await DataProviderFactory.CreateStatisticsDataProvider().GetCountByStatusesStatistics();
            Data = statistics.Select(x=>new StatusValueViewModel(x)).OrderBy(s => s.Status).ToList();
