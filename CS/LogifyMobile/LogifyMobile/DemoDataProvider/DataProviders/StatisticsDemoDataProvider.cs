...
namespace Logify.Mobile.Services.Statistics {
    public class StatisticsDemoDataProvider : IStatisticsDataProvider {
        Random rnd = new Random(GlobalSettings.Instance.SubscriptionId.GetHashCode());

        public async Task<CountByDateStatistics> GetCountByDateStatistics(StatisticsRequestType requestType, DateTime from, DateTime to) {
            ...
            IEnumerable<CountByDate> values = Enumerable.Range(-reportDaysCount + 1, reportDaysCount)
                .Select(x => new CountByDate {
                    ...
                    Count = rnd.Next(requestType == StatisticsRequestType.All ? 100 : 10)
                })
            ...
            return new CountByDateStatistics {
                Total = values.Sum(dv => dv.Count),
                Values = values
            };
        }

        public async Task<IEnumerable<CountByStatus>> GetCountByStatusesStatistics() {
            await Task.Delay(100);
